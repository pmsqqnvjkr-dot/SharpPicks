...
4. **mlb-run-model?force=true** collects fresh MLB data + runs MLB model (12:15 PM, must publish before 12:30 ET first pitches)
5. **model-watchdog** catches silent failures (10:30 AM, 11:30 AM, 1:00 PM)
...
| **mlb_run_model** | **`https://app.sharppicks.ai/api/cron/mlb-run-model?force=true`** | **12:15 PM** |
...
- **10:00 AM** — `run-model?force=true` collects NBA + MLB games, then runs **only the NBA model**
- **12:15 PM** — `mlb-run-model?force=true` re-collects MLB data, then runs **only the MLB model** (must publish before 12:30 ET first pitches)
...
- Three watchdog runs per day provide coverage: 9:30 AM (NBA), 11:30 AM (NBA+MLB), 1:00 PM (all)
- Watchdog NBA threshold: 10 AM ET, MLB threshold: 1 PM ET (grace window after the 12:15 scheduled run)
