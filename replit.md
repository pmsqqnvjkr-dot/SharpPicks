...
### WNBA Model Status — SHELVED (Do Not Ship)
Walk-forward backtest (2026-02-27) across 661 games, 3 seasons (2022-2024): model loses to market at every parameter combination. Best result was 51.0% / -2.7% ROI (shrinkage 0.7, edge 4.0%). Model MAE 10.8 vs Market MAE 8.9 — market closer 63.4% of the time. Root cause is structural: insufficient training data (3 seasons, ~660 games vs NBA's 12+ seasons, ~14,000 games), missing advanced features (no pace/ratings/player-level data), and higher relative volatility (sigma 12.3 on lower-scoring games). The "do not ship" gate worked as designed. Strategy: passively accumulate data via 2025+ live collection pipeline, revisit seriously after 2026 season with 4-5 seasons of data and player-level features (Her Hoop Stats or WNBA API). Future investigation: totals and team-specific situational inefficiencies may be more exploitable than straight spread prediction in thin WNBA markets. Files: `wnba_backtest.py` (walk-forward engine), `wnba_data_pipeline.py` (data backfill), `sport_config.py` (WNBA config — `live: False`).
