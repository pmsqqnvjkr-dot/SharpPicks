...
### WNBA 2025 Live Infrastructure
Rolling ratings are computed after each score collection via `update_wnba_rolling_ratings()` in `main.py`, stored in `wnba_rolling_ratings` SQLite table. Uses 20-game rolling window with 50% prior-season decay, minimum 5 games before rating. Game-day player availability via `check_wnba_star_availability()` cross-references ESPN injury reports against prior-season top-5 players from `wnba_top_players` table. Shadow mode predictions logged to `wnba_shadow_picks` table — every game the model would have picked is recorded with edge, confidence, ratings, player availability, and eventual outcome. Cron endpoints: `/api/cron/wnba-shadow` (run predictions), `/api/cron/wnba-grade` (grade results). CLI: `--wnba-shadow`, `--wnba-grade`, `--wnba-ratings`. 2025 season auto-starts when ESPN/Odds API return games in May. Golden State Valkyries expansion team already in `WNBA_TEAM_ABBR_MAP`.

### Enhanced Markets (NBA)
First-half lines (`spreads_h1`, `totals_h1`) and alternate spreads (`alternate_spreads`) are collected in the standard `/odds` call at no extra API credit cost. Stored in SQLite `games` table columns: `spread_h1_home`, `spread_h1_away`, `spread_h1_home_odds`, `spread_h1_away_odds`, `total_h1`, `spread_h1_home_open`, `total_h1_open`, `alt_spread_minus_{1,3,5,7}`, `alt_spread_plus_{1,3,5,7}`. Player props (points, rebounds, assists) use per-event endpoint at 1 credit per market per region per event — stored in `nba_player_props` SQLite table. CLI: `--props`. Cron: `/api/cron/player-props`. API: `/api/game-board` (Pro-only, returns all enhanced data for today's games).

### Data Architecture
PostgreSQL is used for user data, picks, passes, model runs, bets, referrals, insights, and cron job logs. SQLite stores legacy game data, ML training data, and enhanced market data (1H lines, alt spreads, player props). Tables are append-only for transparency. Data collection includes daily line refreshes, quality checks, and game grading, with all date logic standardized to Eastern Time.
