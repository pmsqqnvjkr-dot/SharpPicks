...
- **Separated rest penalties into two distinct mechanisms:**
  - **All-Star break penalty (`calculate_asb_penalty`):** Fixed 3.0% edge penalty for favorites ≥ 8pt spread on first games back from All-Star break. `is_all_star_first_game()` detects ASB return dates via lookup table (2013–2026). Evidence: 42.4% ATS for big favorites post-ASB (-6.3pp vs baseline). Applied as post-model edge adjustment, not probability. Constants: `ASB_SPREAD_THRESHOLD=8.0`, `ASB_PENALTY_PCT=3.0`.
  - **Generic long rest penalty (`calculate_long_rest_penalty`):** Mild scaling penalty for favorites ≥ 8pt spread with ≥ 5 days rest (non-ASB). Scales at 0.3% per excess day. Only applies when ASB penalty does not (no double-dipping). Evidence: 45.9% ATS for big favorites off 5+ days rest vs 48.7% baseline (-2.8pp). Constants: `LONG_REST_THRESHOLD=5`, `LONG_REST_PENALTY_PER_DAY=0.3`.
  - Both penalties applied as post-model risk adjustments to edge only — ML probabilities remain pure and unaltered.
- **Smooth spread-risk edge weighting:** `spread_risk_adjusted_edge()` replaces the step-function threshold with a continuous discount for spreads > 7pts. Discount rate `SPREAD_RISK_SCALE_RATE=0.25` per 10pts excess, floor at 40%. Both live predictions and walk-forward validation use the smooth curve.
- **Star injury confirmation gate:** `check_star_injury_risk()` penalizes edge by 2% per questionable/doubtful/game-time-decision flag when spread ≥ 10pts. Prevents betting large favorites with uncertain star availability.
- **Pre-tip re-validation cron:** New `/api/cron/pretip-validate` endpoint. Runs 2-3hr before tip-off, re-fetches odds/injuries, re-runs model on published pick. Auto-revokes if edge drops below 1.5%, line drifts ≥ 2pts, or filters no longer pass. `send_revoke_notification()` alerts users. Revoked picks excluded from performance record.
- **Revoked pick UI support:** DashboardTab, PicksTab, PickCard, PickHistoryScreen, and TodayTab show revoked picks with strikethrough, "REVOKED" badge, and reduced opacity.
- **Notification service migrated to FCM:** Replaced OneSignal-based `notification_service.py` with Firebase Cloud Messaging. All push notifications now route through the existing `send_push_to_all` / `send_push_notification` / `send_admin_alert` FCM infrastructure in `app.py`. Lazy imports avoid circular dependency issues.
- **Automated push notifications for picks, passes, and results:** `model_service.py` pick/pass creation triggers push to all Pro users. `grade_pending_picks` now sends result notifications (win/loss/push) after grading. Weekly summary cron job sends push notification alongside emails.
- **Admin health alerts via push:** Data quality checks (`check_data_quality`) send admin push alerts when issues are detected (stale lines, missing spreads, duplicates). Cron failures already alerted via `log_cron` → `send_admin_alert`.
- **New notification functions:** `send_weekly_summary_notification(stats)`, `send_admin_health_alert(title, details)`, and `send_revoke_notification(pick, reason)` added to notification_service.py.

## Previous Changes (February 17, 2026)
- **Capacitor native push notifications:** `firebase.js` detects `Capacitor.isNativePlatform()` and uses `@capacitor/push-notifications` for native iOS/Android APNs/FCM tokens, falling back to web FCM for browsers. `useApi.js` uses full `https://app.sharppicks.ai/api` base URL when running in native wrapper. iOS entitlements (`App.entitlements` with `aps-environment: production`), `UIBackgroundModes: remote-notification` in Info.plist, and Android `POST_NOTIFICATIONS` permission added. `capacitor.config.json` updated with server URL and `PushNotifications` plugin config. Logout uses `apiPost` for native URL compatibility. Note: iOS requires `pod install` and Xcode entitlements linking; Android syncs fully.
